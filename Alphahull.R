...
### Function to convert a-hull into shp 
ahtosp <- function(x, increment=360, rnd=10, proj4string=CRS(as.character(NA))){
    require(alphahull)
    require(maptools)
    if (class(x) != "ahull"){
...
################################################################
# Working directory and file upload

Amauro_r1<- read.csv("C:/Users/vazqu/1.COMPU/Posdoc/Expansion/Amaurochalinus/Presencias_reales/amauro_r1.csv", header=TRUE)

# Alpha hull. I define Alpha and create the polygon (the value of alpha can vary according to the points). 
# In the data there must NOT be duplicated longitude and latitude data, otherwise the function gets ERROR.

alpha=10 
alfa_amauro1<-ahull(x=Amauro_r1$longitude, y=Amauro_r1$latitude, alpha=alpha)
plot(alfa_amauro1)
class(alfa_amauro1)

# transfor from ahull to spatial polygon
ob_shape<-ahtosp(alfa_amauro1)
plot(ob_shape)
class(ob_shape)

# Export shape 
writeOGR(ob_shape, dsn="C:/Users/vazqu/1.COMPU/Posdoc/Expansion/Amaurochalinus/Alphahull", 
         layer="amauro1", driver="ESRI Shapefile", overwrite_layer = TRUE)
